...
sleep 1
Add-HPEiLOUser -Connection $connection -Username QualysAdmin -Password atlpresales -LoginName test -UserConfigPrivilege Yes -iLOConfigPrivilege Yes -RemoteConsolePrivilege Yes -VirtualMediaPrivilege Yes -VirtualPowerAndResetPrivilege Yes -HostBIOSConfigPrivilege Yes -HostNICConfigPrivilege Yes -HostStorageConfigPrivilege Yes -LoginPrivilege Yes
write-host 'Added the QualysAdmin account to the iLO'
sleep 1

#Set IPv4 Settings
$connection | Set-HPEiLOIPv4NetworkSetting -InterfaceType Dedicated -DHCPv4Enabled No -IPv4Address 10.10.124.99 -IPv4Gateway 10.10.1.4 -IPv4SubnetMask 255.255.0.0 -NICEnabled Yes 
write-host 'Changed the IPv4 settings on the iLO, now reseting iLO'
